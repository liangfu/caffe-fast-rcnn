...
template <typename Dtype, bool packed>
  __global__ void caffe_gpu_interp2_kernel(const int n, const float rheight,
      const float rwidth, const int channels, const Dtype *data1,
...
template <typename Dtype, bool packed>
  void caffe_gpu_interp2(const int channels, const Dtype *data1,
      const int x1, const int y1, const int height1, const int width1,
      const int Height1, const int Width1,
      Dtype *data2,
      const int x2, const int y2, const int height2, const int width2,
      const int Height2, const int Width2) {
    CHECK(x1 >= 0 && y1 >= 0 && height1 > 0 && width1 > 0 && x2 >= 0
    ...
    caffe_gpu_interp2_kernel<Dtype, packed><<<CAFFE_GET_BLOCKS(num_kernels),
...
template <typename Dtype, bool packed>
  void caffe_gpu_interp2_backward(const int channels, Dtype *data1,
      const int x1, const int y1, const int height1, const int width1,
      const int Height1, const int Width1,
      const Dtype *data2,
      const int x2, const int y2, const int height2, const int width2,
...
template <typename Dtype, bool packed>
  void caffe_gpu_pyramid2(const int channels,
    ...
    for (int l = 0; l < levels; ++l) {
      ...
      const int num_kernels = height2 * width2;
      // NOLINT_NEXT_LINE(whitespace/operators)
      caffe_gpu_pyramid2_kernel<Dtype, packed><<<CAFFE_GET_BLOCKS(num_kernels),
        CAFFE_CUDA_NUM_THREADS>>>
        (num_kernels, channels, data1, height1, width1, data2,
         height2, width2);
      CUDA_POST_KERNEL_CHECK;
      data1 = data2;
      height1 = height2;
